...
public class IntegracionConvert {
	...
	public AlineacionIntegracion alineacionToAlineacionIntegracion(Alineacion a){
		...
		ai.setGolero(jugadorToJugadorIntegracion(a.getGolero()));
		return ai;
	}
	
	public Alineacion alineacionIntegracionToAlineacion(AlineacionIntegracion ai){
		Alineacion a = new Alineacion();
		for (JugadorIntegracion j : ai.getDefensas()) {
			a.getDefensas().add(jugadorIntegracionToJugador(j));
		}
		
		for (JugadorIntegracion j : ai.getDelanteros()) {
			a.getDelanteros().add(jugadorIntegracionToJugador(j));
		}
		
		for (JugadorIntegracion j : ai.getMediocampistas()) {
			a.getMediocampistas().add(jugadorIntegracionToJugador(j));
		}
		
		for (JugadorIntegracion j : ai.getSuplentes()) {
			a.getSuplentes().add(jugadorIntegracionToJugador(j));
		}
		a.setLesionDefensas(jugadorIntegracionToJugador(ai.getLesionDefensas()));
		a.setLesionDelantero(jugadorIntegracionToJugador(ai.getLesionDelantero()));
		a.setLesionGolero(jugadorIntegracionToJugador(ai.getLesionGolero()));
		a.setLesionMediocampistas(jugadorIntegracionToJugador(ai.getLesionMediocampistas()));
		a.setGolero(jugadorIntegracionToJugador(ai.getGolero()));
		return a;
	}
}
