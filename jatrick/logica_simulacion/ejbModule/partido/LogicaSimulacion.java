...
public class LogicaSimulacion {
	...
	private double probabilidadJugadaGol(Alineacion alineacionLocal,
		...
		long promLocal = (RegateATs + RegateMEDs)
				/ (alineacionLocal.getDelanteros().size() + alineacionLocal
						.getMediocampistas().size());
		long promVisitante = (PotenciaDEFs + PotenciaMEDs)
				/ (alineacionVisitante.getDefensas().size() + alineacionVisitante
						.getMediocampistas().size());
	...
	private void probabilidadLesion(Alineacion alineacionLocal,
		...
		if (Math.random() > 0.5) {
			...
			int j = (int) d;
			if (j == 1) {
				ju = eligoJugadorGol(alineacionLocal.getDefensas());
			} else if (j == 2) {
				ju = eligoJugadorGol(alineacionLocal.getMediocampistas());
			} else {
				ju = eligoJugadorGol(alineacionLocal.getDelanteros());
			}
			...
			int j = (int) d;
			if (j == 1) {
				ju = eligoJugadorGol(alineacionVisitante.getDefensas());
			} else if (j == 2) {
				ju = eligoJugadorGol(alineacionVisitante.getMediocampistas());
			} else {
				ju = eligoJugadorGol(alineacionVisitante.getDelanteros());
			}

		}
		if (Math.random() > 0.96){
			pc.crearComentario(ju.getNombre() + " "+ju.getApellido1()+" se duele y sale cojeando del terreno de juego",p, minuto);
		}
	}
	...
	public long probabilidadTarjeta(Alineacion alineacionLocal,
		 ...
		 */
		double probDelL = Math.random() * 6;
		double probMedL = Math.random() * 3;
		double probDefL = Math.random() * 1;
		Jugador jugadorL = null;
		if (probDelL >= probMedL && probDelL >= probDefL) {
			jugadorL = eligoJugadorGol(alineacionLocal.getDelanteros());
		} else if (probMedL > probDelL && probMedL >= probDefL) {
			jugadorL = eligoJugadorGol(alineacionLocal.getMediocampistas());
		} else {
			jugadorL = eligoJugadorGol(alineacionLocal.getDefensas());
		}
		...
		if (probDelV >= probMedV && probDelV >= probDefV) {
			...
			jugadorV = eligoJugadorGol(alineacionVisitante.getDefensas());
		}
	...
	public void probabilidadLesion() {
	...
	private void simularJugada(Partido p, int minuto) {
		...
		if (chanceLocal) {
			...
			if (probG > 0.8) {
				pc.crearComentario("Pared al borde del área para "
						+ p.getLocal().getNombre()
						+ "  Chuta y... GOOOOOLLLLL!!	", p, minuto);
			...
			if (probG > 0.8) {
				pc.crearComentario(
						"Despiste en el area aprovechado por el ataque, GO GO GO GOOOLLLL!!",
						p, minuto);
